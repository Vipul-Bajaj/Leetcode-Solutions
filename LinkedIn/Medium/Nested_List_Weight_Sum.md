# [Home](./../..)/[LinkedIn](./..)/[Medium](./)/Nested_List_Weight_Sum
...
class Solution:
    def depthSum(self, nestedList: List[NestedInteger]) -> int:
        def flatten(nestedlist,depth,s):
            for nestedinteger in nestedlist:
                if nestedinteger.isInteger():
                    s[0] += (nestedinteger.getInteger()*depth)
                else:
                    flatten(nestedinteger.getList(),depth+1,s)
        s = [0]
        flatten(nestedList,1,s)
        return s[0]
```
